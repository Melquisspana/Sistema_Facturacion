<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight dark:text-paper-100">Ajustes de inventario</h2>
            @can('planta.ajustes.crear')
                <a href="{{ route('planta.ajustes.create') }}"
                   class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">Nuevo ajuste</a>
            @endcan
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="GET" class="mb-4 flex flex-wrap items-end gap-3">
                ...
                <div>
                    <label class="{{ $etiquetaFiltro }}">Tipo</label>
                ...
                <div>
                    <label class="{{ $etiquetaFiltro }}">Disponibilidad</label>
                    <select name="disponibilidad" class="{{ $filtro }}">
                        <option value="">Todas</option>
                        @foreach ($disponibilidades as $d)
                            <option value="{{ $d->value }}" @selected(request('disponibilidad') === $d->value)>{{ $d->label() }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <a href="{{ route('planta.ajustes.index') }}" class="text-sm text-gray-500 hover:underline dark:text-paper-400">Limpiar</a>
            </form>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-ink-600">
                    <thead class="bg-gray-50 dark:bg-ink-900">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-paper-400">
                            ...
                            <th class="px-4 py-3">Confirmado</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm dark:divide-ink-700">
                        @forelse ($ajustes as $ajuste)
                            <tr class="text-gray-700 dark:text-paper-200">
                                ...
                                <td class="px-4 py-3">{{ $ajuste->fecha->format('d/m/Y') }}</td>
                                <td class="px-4 py-3">
                                    <x-planta.badge :color="$ajuste->tipo->color()">{{ $ajuste->tipo->label() }}</x-planta.badge>
                                </td>
                                <td class="px-4 py-3 text-right">{{ $ajuste->detalles_count }}</td>
                                <td class="px-4 py-3">{{ $ajuste->responsable_nombre ?? '—' }}</td>
                                <td class="px-4 py-3">
                                    <x-planta.badge :color="$ajuste->estado->color()">{{ $ajuste->estado->label() }}</x-planta.badge>
                                </td>
                                <td class="px-4 py-3">{{ $ajuste->creadoPor?->name ?? '—' }}</td>
                                <td class="px-4 py-3">{{ $ajuste->confirmado_en?->format('d/m/Y H:i') ?? '—' }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('planta.ajustes.show', $ajuste) }}"
