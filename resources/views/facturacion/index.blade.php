        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6">
                ...
                @can('create', App\Models\Dte::class)
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        <a href="{{ route('facturacion.create-nota-credito') }}"
                           class="inline-flex items-center px-4 py-2 bg-rose-600 text-white text-sm font-medium rounded-md hover:bg-rose-700">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-3">
                    <span class="text-xs font-medium text-gray-400">Accesos rápidos:</span>
                    @php
                    ...
                    <a href="{{ route('facturacion.index', ['estado' => 'aceptado']) }}" class="{{ $chip }}">Aceptados</a>
                    <a href="{{ route('facturacion.index', ['estado' => 'pendientes_emitir']) }}" class="{{ $chip }}">Pendientes de emitir</a>
                    <a href="{{ route('facturacion.index', ['estado' => 'borrador']) }}" class="{{ $chip }}">En edición</a>
                ...
                <form method="GET" action="{{ route('facturacion.index') }}"
                      x-data="{ desde: '{{ $filtros['fecha_desde'] }}', hasta: '{{ $filtros['fecha_hasta'] }}' }"
                      class="mb-5 rounded-lg border border-gray-200 bg-gray-50/60 p-4 space-y-3">

                    <div>
                        <x-input-label for="q" value="Buscar por número, orden de compra, cliente o sala" />
